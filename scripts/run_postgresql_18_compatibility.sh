...
assert_migrations() {
  ...
  [[ "$count" == "43" ]] || fail migrations "Erwartet 43 Migrationen, gefunden $count."
  local latest
  latest="$(db_scalar "$service" "$database" 'SELECT MAX(version) FROM iscy_schema_migrations;')"
  [[ "$latest" == 0043_* ]] || fail migrations "Die erwartete Migration 0043 fehlt."
}
