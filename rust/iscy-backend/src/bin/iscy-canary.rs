...
fn cmd_import(args: &[String]) -> anyhow::Result<()> {
    ...
    for cve in cves {
        ...
        let accepted = body["accepted"].as_bool().unwrap_or(true);
        let is_valid = body["is_valid"].as_bool().unwrap_or(accepted);
        if !accepted || !is_valid {
            let msg = body["message"].as_str().unwrap_or("Ungültige CVE-ID vom Rust-Backend.");
            anyhow::bail!(
                "Import abgebrochen: CVE {} wurde vom Rust-Backend abgelehnt ({})",
                payload["cve_id"],
                msg
            );
        }
    ...
    while page < max_pages {
        ...
        for v in vulnerabilities {
            ...
            if import_response.status().is_success() {
                let import_payload: serde_json::Value = import_response.json()?;
                let accepted = import_payload["accepted"].as_bool().unwrap_or(true);
                let is_valid = import_payload["is_valid"].as_bool().unwrap_or(accepted);
                if accepted && is_valid {
                    imported_records += 1;
                } else {
                    eprintln!(
                        "WARN: Rust-Import hat CVE {} übersprungen: {}",
                        cve_id,
                        import_payload["message"].as_str().unwrap_or("ungültige CVE-ID")
                    );
                }
            } else {
                anyhow::bail!(
