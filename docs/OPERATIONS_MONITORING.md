...
printf '%s\n' 'REPLACE_WITH_STRONG_HMAC_SECRET' > deploy/monitoring/alertmanager/secrets/iscy-alertmanager-hmac
ISCY_ALERTMANAGER_TOKEN_FILE=./alertmanager/secrets/iscy-alertmanager-token \
ISCY_ALERTMANAGER_HMAC_SECRET_FILE=./alertmanager/secrets/iscy-alertmanager-hmac \
  docker compose -f deploy/monitoring/docker-compose.yml up -d
```
