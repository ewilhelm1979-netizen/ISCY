...
|---|---|
| Backend | Rust service (`rust/iscy-backend`) via `RUST_BACKEND_URL` |
...
- musl-based Linux runtimes for the current local-LLM toolchain baseline
...
After any host OS, Python, compiler, PostgreSQL or Rust backend update:

1. run `python manage.py check`
2. run migrations on a copy/staging database
3. run `cargo test --manifest-path rust/iscy-backend/Cargo.toml`
4. run `python manage.py check_local_llm`
5. run a CVE enrichment smoke test
6. validate `docker compose -f docker-compose.yml -f docker-compose.prod.yml config`
7. only then promote to shared/stable environment

## Rust migration definition of done

A migration increment is considered complete when all of the following are true:

- Django calls the Rust adapter seam (`rust_service`) for the targeted capability in Stage and Production profiles.
- Strict-mode safety is covered (`RUST_STRICT_MODE=True` paths fail closed when backend wiring is missing).
- CI executes both Rust backend tests and Django bridge tests for the capability.
- Operations runbook documents rollback/cutover for the capability (`RUST_BACKEND_URL` + compose profile validation).
- Canary/smoke command exists for the capability (for example CVE import/sync canaries).
